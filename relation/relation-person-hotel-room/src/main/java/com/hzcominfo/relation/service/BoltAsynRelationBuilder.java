...
public class BoltAsynRelationBuilder {
    ...
    private final BlockingQueue<Map<String, Object>> queue = new LinkedBlockingQueue(10_0000);
            ...
            for (int n = 0; n < taskSize; n++) {
                Runnable task = new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            Map<String, Object> tuple = queue.take();
                            while (tuple != null) {
                                ...
                                try {
                                    ...
                                    String lgdm = (String) tuple.remove("LGDM");
                                    String fh = (String) tuple.remove("FH");

                                    tuple.put("LGDM_FH", lgdm + "@" + fh);
                                    ...
                                    if (ok) {
                                        tuple.put("type", RelationConfiguration.relationType);
                                        String key = UUID.randomUUID().toString();
                                        messages.put(key, new KeyedMessage(RelationConfiguration.kafkaTarget, key, encoder.encode(new BasicBSONObject(tuple))));
                                    ...
                                    if (count % 1_000 == 0) {
                                        logger.info(logBuilder.delete(0, logBuilder.length()).append(count).append(" -- success --> ").append(tuple).append("-- average spend time ").append((System.currentTimeMillis() - startTime) / 1_000).append(" ms").toString());
