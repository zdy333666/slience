 ...
 * @author llz
 */
public class TupleCheckerDFK {

	private static final Logger logger = LoggerFactory.getLogger(TupleCheckerDFK.class);
    ...
    public static Map<String, Object> check(Map<String, Object> doc) {
        ...
        try {
            String CASENO = (String) doc.get("R_AJBH");
            if (CASENO == null || CASENO.trim().isEmpty() || !CASENO.startsWith("A")) {
                if (counter.incrementAndGet() % sampleSize == 0) {
                    logger.info(logBuilder.delete(0, logBuilder.length()).append("--【R_AJBH:").append(CASENO).append("】-- ").append(" --- is unavailable").toString());
                }
                return null;
            }

            String SFZH = (String) doc.get("R_RYBH_SFZH");
            if (verifySFZH(SFZH) == false) {
                if (counter.incrementAndGet() % sampleSize == 0) {
                    logger.info(logBuilder.delete(0, logBuilder.length()).append("--【R_RYBH_SFZH:").append(SFZH).append("】-- ").append(" --- is unavailable").toString());
                }
                return null;
            }

            String XM = (String) doc.get("R_RYBH_XM");
