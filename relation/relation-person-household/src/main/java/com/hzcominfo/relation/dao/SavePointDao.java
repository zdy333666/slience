...
public class SavePointDao {
    ...
    public void set(SavePoint savePoint) {
        ...
        try {
            ...
            source.getClient().getDB(source.getClientURI().getDatabase()).getCollection(RelationConfiguration.relationSavepointTable).update(query, new BasicDBObject().append("$set", doc), true, false);
    ...
    public SavePoint get(SavePoint savePoint) {
        ...
        try {
            MongoSource source = MongoSourceFactory.getSource(RelationConfiguration.mongo_uri_rtdb);
            DBObject doc = source.getClient().getDB(source.getClientURI().getDatabase()).getCollection(RelationConfiguration.relationSavepointTable).findOne(query, fields);
            if (doc != null) {
                savePoint.setValue((String) doc.get("value"));
