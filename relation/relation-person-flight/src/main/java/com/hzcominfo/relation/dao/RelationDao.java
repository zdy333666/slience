...
public class RelationDao {
    ...
    public boolean build(Session session, Map<String, Object> param) {
        ...
        boolean ok = false;
        String query = "MERGE (p:Person {SFZH:{SFZH}}) ON CREATE SET p.XM={XM}  MERGE (f:Flight {FLIGHT_OFFDAY:{FLIGHT_OFFDAY}})  MERGE (p)-[r:PASSENGER {STRT:{STRT},DEST:{DEST},SEAT:{SEAT}}]->(f) ON CREATE SET r.created=timestamp()";
